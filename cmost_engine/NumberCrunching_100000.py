...
import math
import os
import state34 as _S34   # 34-state 정의 (single source of truth, 기록 전용)
...
def NumberCrunching_100000(p, StageVariables, Location, Cost, CostStage, risc,
                           ...
                           state_recorder=None, quarterly_recorder=None,
                           polyp_recorder=None, state34_recorder=None,
                           colo_flag_recorder=None, colo_probe_recorder=None):
    """
    ...
    # ── state_recorder 헬퍼: POMDP 상태 인덱스 반환 ──────────────────────────
    # state_recorder가 None이 아닐 때만 사용 (성능 영향 없음)
    _SR_IDX = {
        'Normal': 0,
        'P1': 1, 'P2': 2, 'P3': 3, 'P4': 4, 'P5': 5, 'P6': 6,
        'U1': 7, 'U2': 8, 'U3': 9,  'U4': 10,
        'D1': 11,'D2': 12,'D3': 13, 'D4': 14,
        'Dead_CRC': 15, 'Dead_Comp': 16, 'Dead_Other': 17,
    }

    def _pomdp_state_idx(z_):
        """환자 z_의 현재 POMDP 상태 인덱스 반환."""
        if not Alive[z_]:
            cause = int(DeathCause[z_])
            if cause == 2:   return _SR_IDX['Dead_CRC']
            elif cause == 3: return _SR_IDX['Dead_Comp']
            else:            return _SR_IDX['Dead_Other']
        if not Included[z_]:
            cause = int(DeathCause[z_])
            if cause == 2:   return _SR_IDX['Dead_CRC']
            elif cause == 3: return _SR_IDX['Dead_Comp']
        ...
        # 탐지된 암 (D 상태) — 최우선
        n_det = _count_nonzero(Detected_Cancer[z_, :])
        if n_det > 0:
            max_s = int(np.max(Detected_Cancer[z_, :n_det])) - 6
            return _SR_IDX[f'D{max(1, min(4, max_s))}']
        # 미탐지 암 (U 상태)
        n_ca = _count_nonzero(Ca_Cancer[z_, :])
        if n_ca > 0:
            max_ca = int(np.max(Ca_Cancer[z_, :n_ca])) - 6
            return _SR_IDX[f'U{max(1, min(4, max_ca))}']
        # 폴립 (P 상태)
        n_poly = _count_nonzero(Polyp_Polyps[z_, :])
        if n_poly > 0:
            max_p = int(np.max(Polyp_Polyps[z_, :n_poly]))
            return _SR_IDX[f'P{max(1, min(6, max_p))}']
        # Normal
        return _SR_IDX['Normal']

    def _pomdp_state_idx34(z_, now):
        """환자 z_ 의 34-state 인덱스 반환 (기록 전용, 동역학 무관).

        축: surveillance tier(Normal), burden(P), 진단후 phase(D). 정의는
        state34 모듈(_S34)에서 가져와 drift 방지.
        now = time = y + (q-1)/4  (진단후 경과연수 계산용).
        """
        if (not Alive[z_]) or (not Included[z_]):
            cause = int(DeathCause[z_])
            if cause == 2:   return _S34.DEAD_CRC
            elif cause == 3: return _S34.DEAD_COMP
            else:            return _S34.DEAD_OTHER
        # 검출 암 D — 최우선. 병기 × 진단후 phase.
        n_det = _count_nonzero(Detected_Cancer[z_, :])
        if n_det > 0:
            stage = max(1, min(4, int(np.max(Detected_Cancer[z_, :n_det])) - 6))
            yrs = Detected_CancerYear[z_, :n_det]
            pos = yrs[yrs > 0]
            dx = float(np.min(pos)) if pos.size > 0 else now
            return _S34.d_idx(stage, _S34.d_phase(now - dx))
        # 미검출 암 U — 병기 × sojourn(발병후 경과; 최대병기 암 중 가장 오래된 것)
        n_ca = _count_nonzero(Ca_Cancer[z_, :])
        if n_ca > 0:
            cancers = Ca_Cancer[z_, :n_ca]
            mx = int(np.max(cancers)); stage = max(1, min(4, mx - 6))
            yrs = Ca_CancerYear[z_, :n_ca]
            form = yrs[cancers == mx]; form = form[form > 0]
            soj = (now - float(form.min())) if form.size > 0 else 0.0
            return _S34.u_idx(stage, _S34.u_soj(soj))
        # 폴립 P — 최대병기 × dwell(최대병기 폴립 중 가장 오래된 것의 나이)
        n_poly = _count_nonzero(Polyp_Polyps[z_, :])
        if n_poly > 0:
            polyps = Polyp_Polyps[z_, :n_poly]
            mx = int(np.max(polyps))
            stage = max(1, min(6, mx))
            yrs = Polyp_PolypYear[z_, :n_poly]
            form = yrs[polyps == mx]           # 최대병기 폴립들의 형성연도
            form = form[form > 0]
            dwell = (now - float(form.min())) if form.size > 0 else 0.0   # 가장 오래된=최대 dwell
            return _S34.p_idx(stage, _S34.p_dwell(dwell))
        # Normal — surveillance tier (제거 이력)
        if Last_AdvPolyp[z_] != -100:  return _S34.normal_idx(2)  # post_HR
        if Last_Polyp[z_]    != -100:  return _S34.normal_idx(1)  # post_LR
        return _S34.normal_idx(0)                                 # naive

    # ── 비파괴 가상 콜로 probe (실측 콜로 연산자 R용, 동역학 무손) ──────────────
    # CMOST Colonoscopy 규칙을 **별도 RNG**로 시뮬레이션(실제 배열 미수정) → 콜로 직후 state40.
    #   폴립: 제거확률 Colo_Detection[병기]×Location_ColoDetection[위치], reach gate(loc>=reach)
    #   암  : 검출확률 Colo_Detection[병기], reach gate.  (CMOST13 값 그대로 per-lesion)
    # 잔존 폴립은 dwell(형성연도) 유지, 검출암→D_ph0, 미검출암→U 유지, 완전청소→naive.
    _probe_rng = np.random.RandomState(20260707)
    _CD  = np.asarray(StageVariables['Colo_Detection'], dtype=float)
    _LCD = np.asarray(Location['ColoDetection'], dtype=float)

    def _virtual_colo_state34(z_, now):
        if (not Alive[z_]) or (not Included[z_]):
            c = int(DeathCause[z_])
            return _S34.DEAD_CRC if c == 2 else (_S34.DEAD_COMP if c == 3 else _S34.DEAD_OTHER)
        if _count_nonzero(Detected_Cancer[z_, :]) > 0:      # 이미 진단됨 → 콜로 무효
            return _pomdp_state_idx34(z_, now)
        reach = int(ColoReachMatrix[_probe_rng.randint(1000)])   # 도달지점(1-based); loc>=reach 도달
        # 암 검출 (U → D). 미검출 잔존은 병기 × sojourn(발병후 경과) 유지.
        n_ca = _count_nonzero(Ca_Cancer[z_, :]); det = 0; resid = 0; resid_yr = 0.0
        for i in range(n_ca):
            cst = int(Ca_Cancer[z_, i]); cloc = int(Ca_CancerLocation[z_, i])
            if (cloc >= reach) and (_probe_rng.rand() < _CD[cst - 1]):
                det = max(det, cst - 6)
            elif cst - 6 > resid:
                resid = cst - 6; resid_yr = float(Ca_CancerYear[z_, i])
        if det > 0:
            return _S34.d_idx(max(1, min(4, det)), 0)
        if resid > 0:
            soj = (now - resid_yr) if resid_yr > 0 else 0.0
            return _S34.u_idx(max(1, min(4, resid)), _S34.u_soj(soj))
        # 폴립 제거 → 생존자
        n_poly = _count_nonzero(Polyp_Polyps[z_, :])
        surv_st = []; surv_fy = []
        for i in range(n_poly):
            st = int(Polyp_Polyps[z_, i]); loc = int(Polyp_PolypLocation[z_, i])
            if (loc >= reach) and (_probe_rng.rand() < _CD[st - 1] * _LCD[loc - 1]):
                continue                                    # 제거됨
            surv_st.append(st); surv_fy.append(float(Polyp_PolypYear[z_, i]))
        if surv_st:
            smax = max(surv_st)
            fys = [surv_fy[k] for k in range(len(surv_st)) if surv_st[k] == smax and surv_fy[k] > 0]
            dwell = (now - min(fys)) if fys else 0.0
            return _S34.p_idx(smax, _S34.p_dwell(dwell))
        return _S34.normal_idx(0)                            # 완전청소 → naive
    ...
    while np.sum(Included) > 0 and y < 100:
        ...
        for z in range(n):  # z is 0-based (MATLAB z=1:n)
            for q in range(1, 5):  # q = 1,2,3,4
                ...
                # ── quarterly_recorder (A1): 분기 시작 시점 POMDP 상태 스냅샷 ──
                # 동역학 무손 — 기록만 추가. qi = yi*4 + (q-1), t=0..399.
                # 연속 분기경계 스냅샷 → 분기별 전이행렬(399스텝) 도출용.
                if quarterly_recorder is not None:
                    qi_ = yi * 4 + (q - 1)
                    if qi_ < quarterly_recorder.shape[0]:
                        quarterly_recorder[qi_, z] = _pomdp_state_idx(z)

                # ── state34_recorder: 분기 시작 34-state 스냅샷 (기록 전용) ──
                if state34_recorder is not None:
                    qi34 = yi * 4 + (q - 1)
                    if qi34 < state34_recorder.shape[0]:
                        state34_recorder[qi34, z] = _pomdp_state_idx34(z, time)
        ...
        # ── state_recorder: 연간(annual) POMDP 상태 스냅샷 ──────────────────
        # NumberCrunching 루프가 for z: for q 구조라 분기별 기록은 불가능
        # → year y의 4분기 완료 후 yi = y-1 슬롯에 1번 기록
        # state_recorder.shape = (100, n) — annual
        if state_recorder is not None:
            yi_ = y - 1
            if yi_ < state_recorder.shape[0]:
                for z_ in range(n):
                    state_recorder[yi_, z_] = _pomdp_state_idx(z_)

        # ── polyp_recorder (B2/B1): 연말 병기별 폴립 개수(multiset) 기록 ──
        # polyp_recorder.shape = (100, n, 6) — [yi, z, stage-1] = P{stage} 개수
        # 오프라인에서 CMOST 검출규칙 적용해 empirical T_colo(잔존폴립 반영) 유도.
        if polyp_recorder is not None:
            yi_ = y - 1
            if yi_ < polyp_recorder.shape[0]:
                for st in range(1, 7):
                    polyp_recorder[yi_, :, st - 1] = (Polyp_Polyps == st).sum(axis=1)

        # ── colo_flag_recorder: 이 해에 대장내시경을 받았는가 (연 단위, 기록 전용) ──
        # Last_Colonoscopy[z] == y  → 올해 colo 발생. Phase3에서 post-tier 자연진행을
        # 측정할 때 검진 해(year)를 제외하는 데 사용.
        if colo_flag_recorder is not None:
            yi_ = y - 1
            if yi_ < colo_flag_recorder.shape[0]:
                colo_flag_recorder[yi_, :] = (Last_Colonoscopy == y).astype(np.int8)

        # ── colo_probe_recorder: 연말 각 환자 [pre, post-가상콜로] state40 (실측 R용) ──
        # shape (100, n, 2): [yi,z,0]=pre(현재 state40), [yi,z,1]=가상콜로 직후 state40.
        if colo_probe_recorder is not None:
            yi_ = y - 1
            if yi_ < colo_probe_recorder.shape[0]:
                for z_ in range(n):
                    colo_probe_recorder[yi_, z_, 0] = _pomdp_state_idx34(z_, float(y))
                    colo_probe_recorder[yi_, z_, 1] = _virtual_colo_state34(z_, float(y))
