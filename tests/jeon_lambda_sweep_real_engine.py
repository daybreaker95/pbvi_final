"""jeon_lambda_sweep_real_engine.py
====================================
Lambda (colo_penalty_qaly) grid search through the REAL NumberCrunching_policy
engine, using the Jeon-2018-only, 12-factor risk score (top-20% within each
sex, binary bucket-mapped onto CMOST's individual_risk pool;
transitions_9state_sex_risk_jeon20pct.npz) -- see jeon_4way_eval.py for the
lambda=0 baseline this sweep extends.
...
Run: python3 tests/jeon_lambda_sweep_real_engine.py
"""
import os
import sys
import json
...
from jeon_4way_eval import build_nhic_population, NHIC_NPZ
...
def main():
    t_start = time.time()
    rows = []
    out_path = os.path.join(RES, 'jeon_lambda_sweep_real_engine.json')
    ...
    with open(os.path.join(RES, 'jeon_lambda_sweep_baseline.json'), 'w') as f:
