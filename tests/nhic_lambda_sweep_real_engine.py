"""nhic_lambda_sweep_real_engine.py
====================================
Lambda (colo_penalty_qaly) grid search through the REAL NumberCrunching_policy
engine, using the NHIC-based (top-20% within each sex) risk stratification
(transitions_9state_sex_risk_nhic20pct.npz) instead of CMOST's native
individual_risk threshold -- see nhic_4way_eval.py for the lambda=0 baseline
this sweep extends.

IMPORTANT: run this alone, not alongside other heavy background jobs. The
lambda=0 4-way run (nhic_4way_eval.py, 4 processes in parallel) took ~9.3
HOURS per scenario at N=1,000,000 -- ~15x slower than this project's own
prior benchmark of ~2200s/1,000,000 for this same engine (lambda_sweep_
real_engine.py's docstring). That benchmark was from a single process
running alone; the 9.3h figure was 4 processes competing for the same CPU
cores. This script deliberately runs ONE process, ONE lambda at a time, at
a moderate N=200,000 (not 1,000,000) to stay within the fast regime.

Each grid point re-trains BOTH sex-specific FiVI policies (~5-10min each)
then runs the real engine at N=200,000 (~expected minutes, not hours, if
this process has the CPU to itself).

Run: python3 tests/nhic_lambda_sweep_real_engine.py
"""
import os
import sys
import json
...
from nhic_4way_eval import build_nhic_population, NHIC_NPZ
...
def run_one(lam, t_start, p_template):
    print(f'--- lambda={lam:.3f} ---', flush=True)
    # Fresh copy of the NHIC-overridden population each time so successive
    # runs don't accumulate state (p dict holds large arrays modified in
    # place by run_cohort's flag-setting, but individual_risk/risk_class are
    # what matter here and are recomputed identically every time from the
    # same seed anyway -- re-deriving is cheap relative to training/sim time
    # and avoids any subtle cross-run contamination).
    np.random.seed(SEED)
...
def main():
    t_start = time.time()
    rows = []
    out_path = os.path.join(RES, 'nhic_lambda_sweep_real_engine.json')

    # no_screen baseline (needed for per-colo efficiency), same NHIC population
    ...
    with open(os.path.join(RES, 'nhic_lambda_sweep_baseline.json'), 'w') as f:
        json.dump(baseline, f, indent=2)

    for lam in LAMBDAS:
        row = run_one(lam, t_start, None)
